...
那么显然，总计算量与$k$无关，仍为约$mn+\frac{n^2}{2}$
...
\sum_{i=1}^{k}\left[m+\left(i-1\right)\frac{n}{k}\right]=km+\frac{\left({k}-1\right)n}{2}
...
当然，上面的分析极大简化了实际场景，例如工具调用的返回结果等均未考虑，因此上面的计算结果仅供参考。但可以肯定的是，工具调用确实带来了额外的、昂贵的价格。并且不仅只是工具调用，当一次长请求拆分为多次短请求时，总花费将大大增加，即使计算量不变。

现如今许多时候，系统提示词或上文会非常长，在各种编码agent中，几次工作就会让上下文快速超过10万token，即$m$很大。在额外花费$\left(k-1\right)m+\frac{\left({k}-1\right)n}{2}$之中，当$m$远大于$n$时，$\left(k-1\right)m$占据了主导地位。也就是说本质上这种高花费直接来自于每次工具调用要重新计算一次输入token的价格。因此，当$m$很大时，需尽可能减小$k$。

## 优化

让模型输出更多东西会使得总计算量增加，同时带来的是响应速度的下降。但在很大程度上，让模型输出更多东西以避免多次调用工具或其他形式的多次请求，是值得的。
